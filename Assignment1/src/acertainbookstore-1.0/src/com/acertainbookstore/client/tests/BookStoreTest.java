...
public class BookStoreTest {
	 ...
	 * 
	 * 4. We also try to buy non-existing books/ invalid ISBN and check that appropriate exceptions are thrown.
	 * 
	 * 5. We also try to buy 5 copies of the book and check that it is not possible to buy and the appropriate exception is thrown
	 */
	@Test
	public void testBuyBooks() {
		...
		booksToAdd.add(new ImmutableStockBook(testISBN, "Book Name", "Author Name", (float) 100, numCpies, 0, 0, 0, false));
		try {
		...
		for (StockBook b : listBooks) {
			if (b.getISBN() == testISBN) {
				assertTrue("Num copies  after buying one copy", b.getNumCopies() == numCpies - buyCopies);
				break;
			}
		}
	 ...
	 * 
	 * 2. We try to retrieve this book by executing getBooks with a list containing testISBN.
	 * 
	 * 3. We also test that getBooks executed with incorrect arguments throws exception.
	 * 
	 */
	@Test
	 ...
	 * 
	 * 4. We also check that the appropriate exception is thrown when rateBook is executed with wrong arguments
	 */
	@Test
	 ...
	 * 
	 * 1. We add two books one with ISBN - testISBN and another with ISBN - testISBN + 1.
	 * 
	 * 2. We rate book with testISBN with rating 5.
	 * 
	 * 3. Then we execute getTopRatedBooks(1) and check that testISBN is returned.
	 * 
	 * 4. Now we rate testISBN with rating 0 this makes its average rating = 2.5 and testISBN+1 book with rating 4(average rating = 4)
	 * 
	 * 5. We again execute getTopRatedBooks(1) and check that testISBN is not returned but testISBN+1 is.
	 * 
	 */
	@Test
	public void testGetTopRatedBooks() {
		...
		Set<StockBook> booksToAdd = new HashSet<StockBook>();
		booksToAdd.add(new ImmutableStockBook(testISBN, "Book Name", "Book Author", (float) 100, 5, 0, 0, 0, false));
		booksToAdd.add(new ImmutableStockBook(testISBN + 1, "Book Name 1", "Book Author", (float) 100, 5, 0, 0, 0, false));
