		...
		if (messageTag == null) {
			...
			switch (messageTag) {
				...
				case ADDBOOKS:
					...
					String listBooksxmlString = BookStoreUtility.serializeObjectToXMLString(bookStoreresponse);
				...
				case LISTBOOKS:
					bookStoreresponse = new BookStoreResponse();
					bookStoreresponse.setList(CertainBookStore.getInstance().getBooks());
					listBooksxmlString = BookStoreUtility.serializeObjectToXMLString(bookStoreresponse);
					response.getWriter().println(listBooksxmlString);
					break;
				...
				case UPDATEEDITORPICKS:
					...
					try {
						...
						String xmlStringEditorPicksValues = BookStoreUtility.extractPOSTDataFromRequest(request);

						Set<BookEditorPick> mapEditorPicksValues = (Set<BookEditorPick>) BookStoreUtility.deserializeXMLStringToObject(xmlStringEditorPicksValues);

						CertainBookStore.getInstance().updateEditorPicks(mapEditorPicksValues);
					} catch (BookStoreException ex) {
						bookStoreresponse.setException(ex);
					}
					listBooksxmlString = BookStoreUtility.serializeObjectToXMLString(bookStoreresponse);
				...
				case GETBOOKS:
					...
					bookStoreresponse = new BookStoreResponse();
					try {
						bookStoreresponse.setList(CertainBookStore.getInstance().getBooks(isbnSet));
					} catch (BookStoreException ex) {
						bookStoreresponse.setException(ex);
					}
					listBooksxmlString = BookStoreUtility.serializeObjectToXMLString(bookStoreresponse);
				...
				case EDITORPICKS:
					...
					try {
						...
						bookStoreresponse.setList(CertainBookStore.getInstance().getEditorPicks(numBooks));
					} catch (BookStoreException ex) {
						bookStoreresponse.setException(ex);
					}
					listBooksxmlString = BookStoreUtility.serializeObjectToXMLString(bookStoreresponse);
