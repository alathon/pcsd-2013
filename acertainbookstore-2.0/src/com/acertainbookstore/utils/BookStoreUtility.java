...
/**
 * BookStoreUtility implements utility methods used by bookstore servers and clients
 * 
 */
public final class BookStoreUtility {
	...
	/**
	 * Converts a string to a float if possible else it returns the signal value for failure passed as parameter
	 * 
	 * @param str
	 * @param failureSignal
	 * @return
	...
	/**
	 * Converts a string to a int if possible else it returns the signal value for failure passed as parameter
	 * 
	 * @param str
	 * @param failureSignal
	 * @return
	...
	public static BookStoreMessageTag convertURItoMessageTag(String requestURI) {
		...
		try {
			BookStoreMessageTag messageTag = BookStoreMessageTag.valueOf(requestURI.substring(1).toUpperCase());
			return messageTag;
	...
	/**
	 * Manages the sending of an exchange through the client, waits for the response and unpacks the response
	 * 
	 * @param client
	 * @param exchange
	 ...
	 */
	public static List<?> SendAndRecv(HttpClient client, ContentExchange exchange) throws BookStoreException {
		int exchangeState;
		try {
			client.send(exchange);
		} catch (IOException ex) {
			throw new BookStoreException(BookStoreClientConstants.strERR_CLIENT_REQUEST_SENDING, ex);
		}
		...
		if (exchangeState == HttpExchange.STATUS_COMPLETED) {
			try {
				BookStoreResponse bookStoreResponse = (BookStoreResponse) BookStoreUtility.deserializeXMLStringToObject(exchange.getResponseContent().trim());
				...
				throw new BookStoreException(BookStoreClientConstants.strERR_CLIENT_RESPONSE_DECODING, ex);
			}
		} else if (exchangeState == HttpExchange.STATUS_EXCEPTED) {
			throw new BookStoreException(BookStoreClientConstants.strERR_CLIENT_REQUEST_EXCEPTION);
		} else if (exchangeState == HttpExchange.STATUS_EXPIRED) {
			throw new BookStoreException(BookStoreClientConstants.strERR_CLIENT_REQUEST_TIMEOUT);
		} else {
			throw new BookStoreException(BookStoreClientConstants.strERR_CLIENT_UNKNOWN);
		}
	}
