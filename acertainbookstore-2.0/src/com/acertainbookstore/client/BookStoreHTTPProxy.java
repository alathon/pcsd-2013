...
/**
 * BookStoreHTTPProxy implements the client level synchronous CertainBookStore API declared in the BookStore class
 * 
 */
public class BookStoreHTTPProxy implements BookStore {
	...
	public List<Book> getEditorPicks(int numBooks) throws BookStoreException {
		...
		String urlString = serverAddress + "/" + BookStoreMessageTag.EDITORPICKS + "?" + BookStoreConstants.BOOK_NUM_PARAM + "=" + urlEncodedNumBooks;
	...
	public void rateBooks(Set<BookRating> bookRating) throws BookStoreException {
		String xmlString = BookStoreUtility.serializeObjectToXMLString(bookRating);
		String urlString = serverAddress + "/" + BookStoreMessageTag.RATEBOOKS;
		
		ContentExchange exchange = new ContentExchange();
		exchange.setMethod("POST");
		exchange.setURL(urlString);
		Buffer content = new ByteArrayBuffer(xmlString);
	...
	public List<Book> getTopRatedBooks(int numBooks) throws BookStoreException {
		String encodedNumBooks = null;
		try {
			encodedNumBooks = URLEncoder.encode(Integer.toString(numBooks), "UTF-8");
		} catch (UnsupportedEncodingException ex) {
			throw new BookStoreException("Unsupported encoding of numbooks", ex);
		}
		
		String urlString = serverAddress + "/" + BookStoreMessageTag.GETTOPRATED + "?" + BookStoreConstants.BOOK_NUM_PARAM + "=" + encodedNumBooks;
